...
function validateUrl(url$1, protocols) {
  try {
...
function getHttpAgent({
  ...
  if (torPort) {
    return new socksProxyAgent.SocksProxyAgent(`socks5h://tor${retry}@127.0.0.1:${torPort}`);
  }
  if (!proxyUrl) {
    return;
  }
  ...
  if (proxyUrl.includes("socks://") || proxyUrl.includes("socks4://") || proxyUrl.includes("socks5://")) {
    return new socksProxyAgent.SocksProxyAgent(proxyUrl);
  }
  if (proxyUrl.includes("http://") || proxyUrl.includes("https://")) {
    if (isHttps) {
      return new httpsProxyAgent.HttpsProxyAgent(proxyUrl);
    }
    return new httpProxyAgent.HttpProxyAgent(proxyUrl);
  }
}
...
function getGasOraclePlugin(networkKey, fetchOptions) {
  ...
  return new ethers.FetchUrlFeeDataNetworkPlugin(gasStationApi, (fetchFeeData, provider, request) => __async$c(this, null, function* () {
    ...
    const fetchReq = new ethers.FetchRequest(gasStationApi);
    ...
    return {
      gasPrice,
      maxFeePerGas: ethers.parseUnits(`${fast.maxFee}`, 9),
      maxPriorityFeePerGas: ethers.parseUnits(`${fast.maxPriorityFee}`, 9)
    };
  }));
}
function getProvider(rpcUrl, fetchOptions) {
  return __async$c(this, null, function* () {
    ...
    const staticNetwork = new ethers.Network(_staticNetwork.name, _staticNetwork.chainId);
    if (ensPlugin) {
...
function saveEvents(_0) {
  return __async$8(this, arguments, function* ({
    ...
    if (!(yield existsAsync(userDirectory))) {
      yield promises.mkdir(userDirectory, { recursive: true });
    }
    yield promises.writeFile(filePath + ".zip", payload);
    yield promises.writeFile(filePath, stringEvents);
  });
}
function loadSavedEvents(_0) {
  return __async$8(this, arguments, function* ({
    ...
    try {
      const events = JSON.parse(yield promises.readFile(filePath, { encoding: "utf8" }));
      return {
        events,
...
function download(_0) {
  return __async$8(this, arguments, function* ({ name, cacheDirectory }) {
    ...
    const data = yield promises.readFile(zipPath);
...
function parseRelayer(value) {
  if (!value || !(value.endsWith(".eth") || validateUrl(value, ["http:", "https:"]))) {
    throw new commander.InvalidArgumentError("Invalid Relayer ETH address or URL");
  }
  return value;
}
function parseAddress(value) {
  if (!value) {
    throw new commander.InvalidArgumentError("Invalid Address");
  }
  try {
...
function parseMnemonic(value) {
  if (!value) {
    throw new commander.InvalidArgumentError("Invalid Mnemonic");
  }
  try {
    ethers.Mnemonic.fromPhrase(value);
  } catch (e) {
    throw new commander.InvalidArgumentError("Invalid Mnemonic");
  }
  return value;
...
function parseKey(value) {
  if (!value) {
    throw new commander.InvalidArgumentError("Invalid Private Key");
  }
  if (value.length === 64) {
    value = "0x" + value;
  }
...
exports.blockSyncInterval = blockSyncInterval;
...
exports.parseRelayer = parseRelayer;
